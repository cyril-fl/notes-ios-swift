...
class _CModal: UIViewController {
...
struct CModal<Content: View>: UIViewControllerRepresentable {
    ...
    init(isPresented: Binding<Bool>, cornerRadius: CGFloat = 0, backgroundColor: UIColor = .clear, @ViewBuilder content: @escaping () -> Content) {
    ...
    func makeUIViewController(context: Context) -> _CModal {
        let viewController = _CModal()
    ...
    func updateUIViewController(_ uiViewController: _CModal, context: Context) {
